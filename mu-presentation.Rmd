...
- Taking just one day in particular for showing evidence of microclimate is more promising. Hence day considered - **`r unique(day_date$date)`** and time considered is: **`r unique(day_date$hour)`**:**`r unique(day_date$mm)`**.

--

- The highest average temperature recorded is `r max(day_date$temp)`°C at `Between 87 and 89 Grattan Street, Carlton` and the location which is nearest to it `3/121 Grattan Street, Carlton `, records the lowest average temperatures among the sensors with `r min(day_date$temp)`°C, even though it's on the same day and time.  
...
Title Header credits: [Monash](https://www.monash.edu/academicpromotion)
